...
export async function POST(request: NextRequest) {
  try {
    ...
    if (typeof error === "object" && error !== null) {
      ...
      if (
        errObj.message === "Authentication token is required" ||
        errObj.message === "Invalid or expired token"
      ) {
        return NextResponse.json({ error: "Unauthorized" }, { status: 401 });
      }
      if (
        errObj.message &&
        (errObj.message.includes("file type") ||
          errObj.message.includes("file size"))
      ) {
...
export async function DELETE(request: NextRequest) {
  try {
    ...
    const photoIndex = user.photos.findIndex(
      (photo: { _id?: unknown }) => String(photo._id) === photoId
    );
    if (photoIndex === -1) {
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const photo = user.photos.find(
      (p: { _id?: unknown }) => String(p._id) === photoId
    );
    if (!photo) {
      return NextResponse.json({ error: "Photo not found" }, { status: 404 });
    }
