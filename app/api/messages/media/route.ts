  ...
  try {
    ...
    if (!isImage && !isVideo) {
      return NextResponse.json(
        {
          error:
            "Unsupported media type. Allowed images: jpg, png, webp, gif. Allowed video: mp4, mov, webm.",
        },
        { status: 400 }
      );
    }
      ...
      return NextResponse.json(
        {
          error: `File too large. Maximum size is ${
            isVideo ? "25MB for video" : "10MB for images"
          }.`,
        },
        { status: 400 }
      );
    ...
    if (!bucket) {
      console.error(
        "Missing AWS_S3_BUCKET_NAME (or AWS_BUCKET_NAME) environment variable"
      );
      return NextResponse.json(
    ...
    const objectKey = `messages/${matchId}/${Date.now()}-${Math.random()
      .toString(36)
      .slice(2)}.${extension}`;
