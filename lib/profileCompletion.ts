...
export function computeProfileCompletion(user: Partial<IUser> & { photos?: any[]; interests?: string[]; lifestyle?: any; verification?: any; location?: any; dateOfBirth?: Date; bio?: string; }): ProfileCompletionResult {
  let photosScore = 0;
  ...
  const lifestyleScore = lifestyleFilled === 0 ? 0 : (lifestyleFilled / lifestyleFields.length) * WEIGHTS.lifestyle;

  const verificationScore = user.verification?.isVerified ? WEIGHTS.verification : 0;

  const total = photosScore + bioScore + interestsScore + locationScore + dobScore + lifestyleScore + verificationScore;
  ...
  const score = Math.round(Math.min(100, percentage * 80 + richnessBonus + verificationBonus));
