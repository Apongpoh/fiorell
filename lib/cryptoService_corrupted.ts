...
export class CryptoService {
  ...
  constructor(config: CryptoConfig) {
    this.config = config;
    this.network = process.env.CRYPTO_NETWORK || "mainnet"; // Default to mainnet for production
  ...
  createPaymentURL(address: string, amount: number, crypto: "bitcoin" | "monero"): string {
    if (crypto === "bitcoin") {
      ...
      return `bitcoin:${address}?amount=${amount}&label=Fiorell%20Premium%20Subscription`;
    } else {
      // Monero URI format: monero:address?tx_amount=xmr_amount&tx_description=description
      return `monero:${address}?tx_amount=${amount}&tx_description=Fiorell%20Premium%20Subscription`;
    }
  }
